...
def plot_vel_def():
    ...
    #diff =  U_Exp_Blasius{i} - U_Exp_Blasius_Mean(i);%m/s
    #U_Exp_Blasius_rms(i) = sqrt(sum((U_Exp_Blasius_Fluct{i}).^2)/size_array);%m/s
    ...
    plt.title("FS: Recovery of Velocity Deficit at Hub Height", size=20)
    plt.xlabel(labelx, size=20)
    plt.ylabel(labely, size=20)
    plt.grid()
    plt.tight_layout()
    #plt.legend( (Turb,TurbBL, TurbFS, PD,PDBL,PDFS, Hub), ('Turbine','Turbine BL','Turbine FS', 'Porous Disk','Porous Disk BL','Porous Disk FS','Simulation Results'),'best')
    #plt.legend( (Turb, TurbFS, PD,PDFS, Hub), ('Turbine','Turbine FS', 'Porous Disk','Porous Disk FS','Simulation Results'),'best')
    plt.legend( (Turb, PD, Hub), ('Model Turbine','Porous Disk','Numerical Model'),'best')
...
def plot_tunnel_plots():
    ...
    plt.figure()
    #plt.rc("font", size=28)#plt.size = '24'
    Hub, = plt.plot(x, u/U_inf, "r", label='Velocity at Hub Height', size=20)
    plt.title("Velocity at Hub Height Along Wind Tunnel", size=20)
    plt.xlabel('Distance Along Tunnel (m)', size=20)
    plt.ylabel(r'Normalized Velocity $U/U_\infty$', size=20)
    ...
    plt.title("Turbulent Kinetic Energy", size=20)
    plt.xlabel(labelx, size=20)
    plt.ylabel(labelk, size=20)
    plt.grid()
    plt.tight_layout()
...
def plot_profiles(Diam):
    ...
    filepath1 = os.path.join("postProcessing", "sets", latest, 
                            name)
    st = ("/media/tom/b7e5b88c-2a56-43b2-8ed7-24c8be25d90f/DATAPlotting/Turbine FS/normvel_turbine_","FS.mat")
    fileturb = Diam.join(st)
    sp = ("/media/tom/b7e5b88c-2a56-43b2-8ed7-24c8be25d90f/DATAPlotting/Porous Disk FS/normvel_disk_",".mat")
    filepd = Diam.join(sp)
   # turb_x = 35 #x-coordinate of the turbine
    D = .25 #Diameter of turbine (meters)
    #Labels to be reused 
    lb = ("FS: Velocity Profile "," behind Turbine")
    ...
    #print half
    u_n = u/U_inf  
    h_n = h/D-2 #Shift down 2D 
    #Plotting
    Sim, = plt.plot( u_n[half:] , h_n[half:], "-+k")
    #Sim, = plt.plot( u_n ,h_n, "-*k")
